...
export default function UpfrontTailKPIs({ data }) {
  const { ui, meta, refKey, totalSOC, totalCollected, totalNet, eMoic, eIrr, irrLabel, pLoss, pHurdle, favorColor } = useKPIData(data);
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: ui.space.md }}>
      ...
      <KPI label={irrLabel} value={fmtPct(eIrr)} sub={refKey ? `at ${refKey.replace('_', '/')}` : ''} color={favorColor(eIrr, 0.25, 0.10)} />
