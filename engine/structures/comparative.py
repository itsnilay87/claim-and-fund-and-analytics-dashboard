"""Comparative structure handler.

Currently uses the same investment-grid analysis as the monetisation types.
The V2 JSON exporter (v2_json_exporter_ext.py) handles comparative-specific
JSON enrichment separately.
"""
...
class ComparativeHandler(StructureHandler):
    ...
    def postprocess_dashboard(self, data, sim, grid, waterfall_grid_results,
        ...
        if not data.get("structure_type"):
            data["structure_type"] = "comparative"
